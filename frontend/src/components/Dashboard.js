...
import { getCurrentUser, getSupabaseUser } from '../services/authService';
    ...
    const fetchUserData = async () => {
      try {
        ...
        if (supabaseSession) {
          ...
          setUserData({
            id: supabaseUser.id,
            username: supabaseUser.user_metadata?.full_name || supabaseUser.email?.split('@')[0] || 'Supabase User',
          ...
          // User is authenticated via backend
          const data = await getCurrentUser();
          setUserData(data);
        }
      } catch (err) {
        setError(err.detail || err.message || 'Failed to fetch user data');
      } finally {
        setLoading(false);
      }
    };
            ...
            <div className="stat-card">
              ...
              <p className="stat-value">
                {userData.auth_method === 'both' ? '🔗 Hybrid' : 
                 userData.auth_method === 'google' ? '🌐 Google OAuth' : 
                 userData.auth_method === 'supabase' ? '⚡ Supabase' : '🔐 Local'}
              </p>
            </div>
          </div>
        </div>
